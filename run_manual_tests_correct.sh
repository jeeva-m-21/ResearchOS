...
echo "=== CORRECT MANUAL TEST RUNNER ==="
echo ""
...
# Test 3: Experiment creation (as QUERY parameters!)
echo "🧪 Test 3: Experiment creation"
EXP_RESPONSE=$(curl -s -X POST "http://localhost:8000/v1/experiments/?name=ManualTestRun&project_id=$PROJECT_ID&description=Testing+from+manual+runner" \
...
if echo "$EXP_RESPONSE" | grep -q '"id"'; then
    ...
    if echo "$RUN_RESPONSE" | grep -q '"id"'; then
        ...
        echo "Metric response: $METRIC_RESPONSE"
...
if [ $tests_failed -eq 0 ]; then
    ...
    echo ""
    echo "✅ CORE WORKFLOW WORKS! ($tests_passed/8 tests passed)"
else
    echo ""
